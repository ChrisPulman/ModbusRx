...
public sealed class SimulationDataProvider : IDisposable
{
    ...
    public static ushort[] GenerateSineWave(int length, double amplitude = 32_767, double frequency = 1.0, double phase = 0.0)
    {
        var result = new ushort[length];
        for (var i = 0; i < length; i++)
        {
            var value = (amplitude * Math.Sin((2 * Math.PI * frequency * i / length) + phase)) + amplitude;
            result[i] = (ushort)Math.Max(0, Math.Min(65_535, value));
        }
    ...
    public static ushort[] GenerateSquareWave(int length, ushort highValue = 65_535, ushort lowValue = 0, double dutyCycle = 0.5)
    {
        var result = new ushort[length];
    ...
    public static ushort[] GenerateSawtoothWave(int length, ushort maxValue = 65_535, ushort minValue = 0)
    {
        var result = new ushort[length];
    ...
    public ushort[] GenerateRandomData(int length, ushort minValue = 0, ushort maxValue = 65_535)
    {
        var result = new ushort[length];
        for (var i = 0; i < length; i++)
        {
        ...
        try
        {
            switch (pattern)
            {
                ...
                case TestPattern.AllOnes:
                    {
                        LoadConstantPattern(dataStore, dataLength, 65_535, true);
                        break;
                    }
            }
        }
    ...
    private static void UpdateCountingData(DataStore dataStore, int count, bool countUp)
    {
        for (var i = 1; i < Math.Min(count + 1, dataStore.HoldingRegisters.Count); i++)
        {
            ...
            dataStore.HoldingRegisters[i] = countUp
                ? (ushort)((currentValue + 1) % 65_536)
                : (ushort)((currentValue == 0) ? 65_535 : currentValue - 1);
    ...
    private static void UpdateSineWaveData(DataStore dataStore, int count)
    {
        ...
        for (var i = 1; i < Math.Min(count + 1, dataStore.HoldingRegisters.Count); i++)
        {
            var value = (32_767 * Math.Sin((2 * Math.PI * 0.1 * time) + (i * 0.1))) + 32_767;
            dataStore.HoldingRegisters[i] = (ushort)Math.Max(0, Math.Min(65_535, value));
            dataStore.InputRegisters[i] = dataStore.HoldingRegisters[i];
            dataStore.CoilDiscretes[i] = dataStore.HoldingRegisters[i] > 32_767;
            dataStore.InputDiscretes[i] = !dataStore.CoilDiscretes[i];
        }
    }
    ...
    private static void UpdateSquareWaveData(DataStore dataStore, int count)
    {
        ...
        for (var i = 1; i < Math.Min(count + 1, dataStore.HoldingRegisters.Count); i++)
        {
            dataStore.HoldingRegisters[i] = isHigh ? (ushort)65_535 : (ushort)0;
            dataStore.InputRegisters[i] = dataStore.HoldingRegisters[i];
    ...
    private static void LoadCountingPattern(DataStore dataStore, int length, bool countUp)
    {
        for (var i = 1; i < Math.Min(length + 1, dataStore.HoldingRegisters.Count); i++)
        {
            ...
            dataStore.HoldingRegisters[i] = (ushort)(value % 65_536);
            dataStore.InputRegisters[i] = dataStore.HoldingRegisters[i];
    ...
    private static void LoadSineWavePattern(DataStore dataStore, int length)
    {
        ...
        for (var i = 1; i < Math.Min(length + 1, dataStore.HoldingRegisters.Count); i++)
        {
            ...
            dataStore.CoilDiscretes[i] = sineData[i - 1] > 32_767;
            dataStore.InputDiscretes[i] = !dataStore.CoilDiscretes[i];
        }
    }
